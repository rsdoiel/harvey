...
func TestChunkDocument_SingleChunk(t *testing.T) {
	...
	if !strings.Contains(chunks[0], "Short document") {
...
func TestChunkDocument_ParagraphSplit(t *testing.T) {
	...
	for _, ch := range chunks {
		if strings.Contains(ch, p1) {
			seen["a"]++
		}
		if strings.Contains(ch, p2) {
			seen["b"]++
		}
		if strings.Contains(ch, p3) {
			seen["c"]++
		}
	}
...
func TestChunkDocument_SourceSplit(t *testing.T) {
	...
	foundBar := false
	for _, ch := range chunks {
		if strings.Contains(ch, "func Foo") {
			foundFoo = true
		}
		if strings.Contains(ch, "func Bar") {
			foundBar = true
		}
	}
	if !foundFoo {
...
func TestChunkDocument_NoOverlap(t *testing.T) {
	...
	for i := 0; i < len(chunks)-1; i++ {
		for _, para := range strings.Split(chunks[i], "\n\n") {
			para = strings.TrimSpace(para)
			if para == "" {
				continue
			}
			if strings.Contains(chunks[i+1], para) {
