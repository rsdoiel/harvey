...
## 2026-07-27 — `knowledge.go` extracted into its own module, `github.com/rsdoiel/knowledge`

**Context.** Step 2 of `knowledge_db_merge_design.md`'s sequencing (UUID migration → SQL merge → module split → JSON-L export, in that order), unblocked by the cross-machine merge completing earlier the same day (previous entry). Motivation: `henry` and `antennaApp` both want to write structured observations into `agents/knowledge.db` but were limited to raw `sqlite3` CLI inserts, since the typed CRUD API was locked inside harvey's own module. Full audit in the new repo's `module-extraction-design.md`; phased plan in `module-extraction-plan.md`.

**Decision.** `knowledge.go`, `knowledge_merge.go`, `knowledge_test.go`, `knowledge_merge_test.go`, and `cmd/kbmerge` all moved verbatim into `~/Laboratory/knowledge` (new repo, scaffolded via CMTools — `codemeta.json`, `cmt --init go`, `LICENSE`, `go.mod`). The only harvey-specific coupling found in the audit was `OpenKnowledgeBase(ws *Workspace, customPath string)`'s dependence on `Workspace.AbsPath` for default-path resolution — replaced with `Open(dbPath string)` plus a `DefaultPath(root string) string` helper, so the library no longer needs to know what a "workspace" is. `IdentifierType` (used by callers) stays in harvey — confirmed unreferenced by the knowledge-base code itself. Kept `KnowledgeBase`/`OpenKnowledgeBase`-style naming as-is rather than also cleaning up the `knowledge.KnowledgeBase` stutter in the same pass — deliberately deferred (see Rejected). `harvey/go.mod` uses `replace github.com/rsdoiel/knowledge => ../knowledge` until the new repo is pushed/tagged.

**Two real gaps surfaced only once compiling harvey against the actual package boundary — not caught by the design doc's audit, which could only inspect source, not enforce it:**
1. `commands_kb.go`'s `/kb observe` called the then-unexported `isValidKind`/used `ValidObservationKinds` — the module's own package boundary made this a real compile error, not a style nit. Exported as `IsValidKind` (with a proper doc comment) in the `knowledge` module.
2. `commands_kb.go`'s `/kb show` reached directly into `KnowledgeBase`'s **unexported** `db` field to hand-run a query fetching one observation by id — an encapsulation break that was completely invisible while both files shared a package, and only became a compile error once separated. Added `ObservationByID` (with its own test) to the `knowledge` module as the proper public API, rather than exposing the field.

A third issue — a leftover `OpenKnowledgeBase(ws, "")` call in `commands_test.go`'s `newTestAgentWithKB` — wasn't caught by `go build` at all (test files aren't part of a build), only surfaced via `go vet`/`go test`. Worth remembering for any future extraction of this shape: `go build` passing is not sufficient evidence that a package-boundary move is complete.

**Rejected.** Renaming `KnowledgeBase`/`OpenKnowledgeBase` to drop the `knowledge.KnowledgeBase` stutter (e.g. `knowledge.Base`/`knowledge.Open`) in the same pass — combining a repo move, a signature change, and a rename into one change makes it harder to tell which part caused a problem if one appears; a pure cosmetic rename is low-risk enough to do later, on its own. Fixing root `CLAUDE.md`'s separate, pre-existing `experiments`/`experiment_*` schema staleness (documented in `knowledge_db_schema_stale.md`) while touching nearby docs — confirmed that section doesn't actually reference `harvey/knowledge.go` or any implementation file at all (it's schema/SQL-only), so there was no "module location" reason to touch it; the older staleness issue is out of this change's scope and wasn't fixed.

**Consequences.** `go build`/`go vet`/`go test` clean in both repos; `bin/harvey` builds. Smoke-tested twice against real data: `bin/kbmerge` (moved, import-only change) merging two copies of the live production db produced identical counts to its pre-move behavior; a throwaway test exercising harvey's actual `initKnowledgeBase` code path against the real, live `agents/knowledge.db` confirmed all 5 real projects (`harvey`, `henry`, `antennaApp`, `sparqlset`, `audiobox`) round-trip correctly through the moved code. `harvey/CLAUDE.md`'s three-silo table updated to name the external module. Still open: publishing `github.com/rsdoiel/knowledge` to GitHub and dropping the `replace` directive; the `KnowledgeBase`/`Open` stutter rename; JSON-L export (step 3 of the sequencing, deferred, unstarted).

---

## 2026-07-27 — Cross-machine `knowledge.db` merge completed and placed on both machines

**Context.** With both fixes verified against a copy of macmini's data (previous entry), completed the actual cross-machine sync this whole thread existed for. First, over SSH: both `~/Laboratory` (root repo) and `~/Laboratory/harvey` on `macmini-rd.local` were behind `origin/main` (root repo was also ahead by one local, unpushed commit touching only `knowledge_tool.md` — no overlap with incoming changes). Fetched and diffed the incoming commits before pulling to confirm no conflicts; `harvey` fast-forwarded cleanly to `8aaaea4`, root `Laboratory` merged cleanly (merge commit `e418c8e`, local commit preserved). Backed up macmini's live `agents/knowledge.db` (`agents/knowledge.db.pre-experiments-migration-20260727`), then triggered both the `concepts.created_at` fix and the `experiments`→`projects` migration against it directly (same throwaway `go run` pattern used on the copy) — verified identical results to the earlier copy-based test (4 projects, 0 unmigrated observations, 0 null `created_at`, legacy tables intact).

**Decision.** `scp`'d macmini's now-migrated live database to wren, ran `bin/kbmerge -a <wren's live db> -b <macmini's live db> -force` locally on wren, and independently verified the output before placing it anywhere: `PRAGMA integrity_check` clean, zero dangling rows across all six join-table combinations, every row has a non-empty `uuid`. Backed up wren's pre-merge live database (`agents/knowledge.db.pre-merge-20260727`), replaced wren's live `agents/knowledge.db` with the merged file, then `scp`'d that same merged file to replace macmini's live `agents/knowledge.db`. Confirmed byte-identical via checksum on both machines afterward (`06c7ecf...`) — true parity, not just "close enough."

**Rejected.** `scp`-ing wren's *original, unmerged* `knowledge.db` directly over macmini's — raised and rejected explicitly (by the user) before doing this, since it would have silently discarded macmini's unique data (the newly-migrated `harvey`/`sparqlset`/`audiobox` project data) instead of merging it in. `bin/kbmerge` never overwrites its inputs by design, so parity requires placing its *output* on both ends, not just moving one input on top of the other.

**Consequences.** Both machines now have an identical, fully-merged `agents/knowledge.db`: 5 projects (`harvey`, `henry`, `antennaApp`, `sparqlset`, `audiobox`), 46 concepts, 181 observations — no data lost (88 from wren + 93 from macmini = 181, exact). Two real name/uuid collisions (`harvey`, `henry` — both genuinely the same real-world project independently tracked on each machine) were found and reconciled via `-force`, same as the dry-run test earlier. Backups exist on both machines at every step (`.pre-cleanup-20260727`, `.pre-experiments-migration-20260727`, `.pre-merge-20260727`) in case anything needs to be unwound. Not pushed to git: macmini's root `Laboratory` merge commit `e418c8e` remains local-only (`agents/knowledge.db` itself is untracked, unaffected by this). Remaining steps from `../knowledge_db_merge_design.md`'s sequencing — knowledge-base module extraction (step 3), JSON-L export (step 4, deferred) — are unstarted; not part of this session's scope.

---

## 2026-07-27 — `experiments` → `projects` legacy data migration; real cross-machine `bin/kbmerge` succeeds end-to-end

**Context.** The `concepts.created_at` fix (previous entry) got a real `bin/kbmerge` attempt against macmini's data past `concepts` and into the next, deferred blocker: `no such column: o.project_id`. Auditing this (rather than assuming "harvey renamed experiments to projects and forgot a migration") found the actual origin: `harvey/knowledge.go` has used `projects`/`project_id` since its very first commit (`5b1d802`) — it has never contained the word "experiment." The `experiments`/`experiment_concepts`/`experiment_summary` schema documented in root `CLAUDE.md` describes an older, hand-run `sqlite3` convention that predates `harvey`'s SQL-backed implementation and was never updated once `harvey` superseded it (matches the existing `knowledge_db_schema_stale.md` memory, filed 2026-07-05, which flagged this as a docs staleness issue — this is the same gap showing up as live, real data on one machine). Checked every reachable `knowledge.db` (wren's live file, `harvey/agents/knowledge.db`, `harvey/.harvey/knowledge.db`, and the copy of macmini's file): only macmini's has `experiments` tables, holding 93 real observations and 35 concepts across 4 real projects (`harvey`, `sparqlset`, `henry`, `audiobox`) invisible to current `harvey` (nothing in `knowledge.go` reads `experiments`). `henry` existed in *both* `experiments` (real data) and `projects` (an empty stub, created at some point after `harvey` started being used on that machine) — the central collision case the migration had to resolve correctly.

Full audit and confirmed decisions in `experiments-migration-design.md`; phased TDD plan in `experiments-migration-plan.md`.

**Decision.** `migrateExperimentsToProjects` (new function in `knowledge.go`, called from `OpenKnowledgeBase` right after `kb` is constructed): (1) no-op, checked via `sqlite_master`, if no `experiments` table exists — the common case, unaffected; (2) for each `experiments` row, match `name` against an existing `projects` row and reuse it if found (resolves the `henry` collision — its real observations attach to the *pre-existing* stub project, not a new duplicate), else create one via the existing `AddProject` path, folding `language`/`repo_url` into `description` (e.g. `"Go — https://github.com/rsdoiel/Laboratory"`) since `projects` has no equivalent columns; (3) translate `observations.experiment_id` → the resolved `project_id`, guarded by `project_id IS NULL` for idempotency; (4) translate `experiment_concepts` → `project_concepts` via `INSERT OR IGNORE`, deduped by the join table's own primary key. Added `ALTER TABLE observations ADD COLUMN project_id INTEGER REFERENCES projects(id)` to `kbAlterStmts` (nullable, no default — unlike the `concepts.created_at` case this needed no special handling, since a plain `NULL` default has no non-empty-table restriction). **`experiments`/`experiment_concepts`/`experiment_summary` are never modified or dropped** — inert once migrated, sidestepping `observations.experiment_id`'s `NOT NULL REFERENCES experiments(id)` constraint entirely rather than attempting a `DROP COLUMN`.

TDD-first: five tests in `knowledge_test.go` (`TestOpenKnowledgeBase_MigratesExperimentsWithoutCollision`, `_WithCollision`, `_MigrationIdempotent`, `_MigrationLeavesLegacyTablesIntact`, `_NoExperimentsTableIsNoOp`) — three confirmed red (missing project creation, missing `project_id` column) before implementation, all five green after, on the first implementation attempt.

**Rejected.** Dropping the legacy tables/column after migration — no correctness benefit, one-way, and avoids the `DROP COLUMN`/FK complication entirely (see design doc decision 5). Inventing a new `projects` column for `language`/`repo_url` — unnecessary for data that exists on exactly one legacy database and is superseded going forward by `codemeta.json`-based tracking anyway (folding into `description` was your explicit call over the alternative of just dropping it).

**Consequences.** `go vet ./...` and `go test ./...` clean. Verified against the real copy of macmini's `knowledge.db` (`macmini-rd.local-agents/knowledge.db` on wren, migration triggered via a throwaway `go run`, then removed): 3 new projects created (`harvey`, `sparqlset`, `audiobox`) with folded descriptions, `henry`'s pre-existing description left untouched, zero unmigrated observations, legacy tables intact and unchanged, and the observation/concept counts per project (69/10/9/5, summing to the expected 93) match the pre-migration `experiments` data exactly. **Then ran the real cross-machine merge this whole thread exists for:** `bin/kbmerge -a <wren's live db> -b <migrated macmini copy> -force` succeeded completely — found and reconciled 2 real name/uuid collisions (`harvey` and `henry`, both genuinely the same real-world project independently tracked on each machine — `harvey` colliding was new information, only possible once macmini's `harvey` project existed at all), merged 3+4→5 projects, 17+35→46 concepts, 87+93→180 observations (no data lost — 87+93 sums exactly to 180), and wrote a reviewable merged database. Not yet done: macmini's actual **live** `agents/knowledge.db` still needs this same code fix applied via its own `git pull` + reopen (only the copy on wren was exercised); the merged output database has not been copied into place over either machine's real file — `bin/kbmerge` deliberately never does that automatically, per its own design.

---

## 2026-07-27 — `concepts.created_at` lazy-migration gap fixed; real cross-machine `bin/kbmerge` attempt surfaces the next (deferred) blocker

**Context.** With both machines UUID-migrated (previous entry) and a real copy of macmini's `agents/knowledge.db` placed on `wren` (`macmini-rd.local-agents/knowledge.db`, copied by the user via the `agents/` directory tree), ran an actual `bin/kbmerge -a <wren's live db> -b <macmini's copy> -force`. It failed: `knowledge: merge concepts from b: SQL logic error: no such column: created_at`. Auditing macmini's live schema (over SSH, before the copy existed) had already shown its `concepts` table predates `created_at` being added to the base `CREATE TABLE` DDL in `schema` (`knowledge.go`) — and `kbAlterStmts` had no `ALTER TABLE concepts ADD COLUMN created_at ...` to backfill it, an oversight from whenever `created_at` was added to the DDL (every other column added since then — `identifier_type`, `identifier_value`, `uuid`, `origin_host` — got a matching `ALTER`; `created_at` did not). Nothing in the live app ever reads `concepts.created_at` (confirmed via `grep`), so this was invisible until `knowledge_merge.go`'s hardcoded column list for the `concepts` copy step needed it.

**Decision.** Added `ALTER TABLE concepts ADD COLUMN created_at DATETIME` to `kbAlterStmts` (no `DEFAULT CURRENT_TIMESTAMP` clause) plus a new one-time, idempotent `UPDATE concepts SET created_at = CURRENT_TIMESTAMP WHERE created_at IS NULL` run right after the `kbAlterStmts` loop. TDD-first per convention: `TestOpenKnowledgeBase_BackfillsLegacyConceptsCreatedAt` (`knowledge_test.go`) builds a raw legacy-shape `concepts` table (no `created_at`) matching macmini's real schema, seeds one row, then opens it via `OpenKnowledgeBase` and asserts `created_at` is backfilled to a non-empty value — confirmed red (`no such column: created_at`) before the fix, green after.

**A second, more interesting bug surfaced while diagnosing the first attempt at this fix**, worth recording since it explains why the fix isn't a plain `ADD COLUMN ... DEFAULT CURRENT_TIMESTAMP`: SQLite itself (not a driver quirk — reproduced identically against the real `sqlite3` CLI) rejects `ALTER TABLE ADD COLUMN` with a non-constant default (`CURRENT_TIMESTAMP`, `CURRENT_DATE`, `CURRENT_TIME`, or any expression) **whenever the table already has at least one row** — confirmed by bisecting five probe tests (empty table: succeeds; table with one pre-existing row: `Cannot add a column with non-constant default`). Every real-world legacy `concepts` table this fix will ever run against necessarily has rows (that's the whole reason the column is missing), so the original naive fix (mirroring the DDL's own `DEFAULT CURRENT_TIMESTAMP`) would have failed at the exact moment it mattered and been masked again by `kbAlterStmts`' blanket `_, _ = db.Exec(stmt)` error-swallowing — exactly the kind of silent-migration-failure this project's convention of writing a red test against the real reproduction case first is meant to catch.

**Rejected.** Giving the `ALTER` statement `DEFAULT CURRENT_TIMESTAMP` directly — fails on any non-empty table, i.e. every real one. Rewriting `AddConceptWithIdentifier`'s `INSERT` to explicitly supply `created_at` — unnecessary: fresh tables (created via the current `schema` DDL) already get `DEFAULT CURRENT_TIMESTAMP` at the column-definition level for free, and `concepts.created_at` isn't read anywhere in the live app today, so a legacy-altered table's new rows briefly showing `NULL` until the next session's backfill catches up is an acceptable, low-stakes gap for a column with no consumer — consistent with the project's own precedent of using best-effort sentinels (`origin_host="unknown"`) rather than fabricating precise provenance for pre-existing data.

**Consequences.** `go vet ./...` and `go test ./...` clean. Re-ran the real cross-machine merge after this fix and after re-triggering the lazy migration on the (pre-fix-snapshot) copy of macmini's db: it now gets **past** the `concepts` step and fails at `observations` instead — `no such column: o.project_id` — which is the already-known, already-deferred `experiments`/`experiment_id` vs `projects`/`project_id` legacy-schema gap (see `TODO.md`; not addressed here, per explicit scope decision to fix only `concepts.created_at` today). This confirms the fix is complete and correctly scoped: the merge tool now fails at the next real, distinct, already-documented blocker, not a re-occurrence of this one. Macmini's actual live `agents/knowledge.db` (as opposed to the copy on `wren`) still needs this same code fix applied via its own `git pull` + reopen — not done in this session, since it wasn't asked for and only the copy was available for testing.

---

## 2026-07-27 — Correction: 2026-07-26 UUID migration work happened on `wren`, not `macmini-rd.local`; macmini migrated separately, genuinely divergent data confirmed

**Context.** Following the same-day cleanup below, checked `macmini-rd.local` over SSH for the same dangling-row issue and to confirm its UUID migration state, since `TODO.md`/`DECISIONS.md`'s 2026-07-26 entries said the migration was "verified on this machine (macmini-rd.local)". Its live `agents/knowledge.db` had **no `uuid` column at all** — the migration had never run there. Cross-referencing timestamps (the `bin/kbmerge` build time, the db file mtime, and the `origin_host='wren'` rows already found on wren) makes clear the entire 2026-07-26 UUID-migration/merge-tool thread actually ran on `wren`; "macmini-rd.local" in the prior write-up was a mislabel (probably carried over from the design docs, which use `macmini-rd.local` as the example machine name throughout), not a real machine identification.

**Decision.** `git pull` on macmini (was one commit behind, clean tree, fast-forwarded to `394b499`), then triggered the migration directly: wrote a throwaway `cmd/migrateonce/main.go` that calls `harvey.NewWorkspace` + `harvey.OpenKnowledgeBase` against the real `/Users/rsdoiel/Laboratory` workspace, ran it once with `go run`, then deleted the directory (`git status` confirmed clean afterward — no trace left in the repo). Chose this over launching `bin/harvey` interactively over SSH, which would have blocked on stdin.

**Consequences.** Macmini's `agents/knowledge.db` is now migrated: all rows in `projects`/`observations`/`concepts` have non-empty `uuid`s (macmini has no `sources` rows yet). Confirmed macmini has **zero dangling join-table rows** already — the historical-delete bug fixed below is specific to wren's file, not universal. More importantly, macmini's data is confirmed genuinely divergent from wren's: macmini has only the `henry` project (35 concepts, 93 observations, 0 sources), while wren has `harvey`/`henry`/`antennaApp` (17 concepts, 85 observations, 6 sources) — a real test case for `bin/kbmerge`, not the same-machine stand-in used in the 2026-07-26 verification. Both machines are now genuinely migrated; the next actionable step for `../knowledge_db_merge_design.md`'s sequencing is transferring one machine's `knowledge.db` to the other (or running `kbmerge` with both files reachable from one machine) to do a first real cross-machine merge.
...
**Decision.** `MergeKnowledgeBases(aPath, bPath, mergedPath string) ([]MergeTableSummary, error)` in a new `knowledge_merge.go` (package `harvey`): creates a fresh, fully-migrated `mergedPath` by reusing `OpenKnowledgeBase`, `ATTACH DATABASE`s both sources read-only, copies parent tables (`projects`, `concepts`, `sources`) with an explicit column list excluding `id` so dedup happens only via the real `uuid`/`name` UNIQUE constraints (not an accidental raw-id collision — see the correction below), translates `observations.project_id` and all three join tables through `uuid` joins on both sides, and reopens `mergedPath` once more afterward so the existing `rebuildFTSIfNeeded` (`knowledge.go:811`) populates the merged FTS index for free. A separate `CollisionReport(aPath, bPath) ([]NameCollision, error)` surfaces the known pre-migration edge case (same `projects.name`/`concepts.name`, two different `uuid`s) before a real merge runs, rather than letting `INSERT OR IGNORE` resolve it silently. Orchestration (WAL checkpoint, file copy with `-wal`/`-shm` sidecars, backup, printing the summary) lives in a new `cmd/kbmerge` binary — core logic stays in the tested package, mirroring `cmd/assay`'s relationship to the root package.

**Correction to the umbrella design, caught while writing this doc.** `../knowledge_db_merge_design.md`'s original parent-table step said `INSERT OR IGNORE INTO merged.<table> SELECT * FROM a.<table>`. Since `id` is `INTEGER PRIMARY KEY AUTOINCREMENT` in every table, a literal `SELECT *` would copy `a`'s raw ids into `merged` first; `b`'s copy running afterward could then be silently dropped by `INSERT OR IGNORE` whenever a `b` row's raw id happened to coincide with an already-inserted `a` row — a false dedup on primary key, not the intended one on `uuid`. Fixed by using an explicit column list that excludes `id` everywhere, letting `merged` assign fresh autoincrement ids per row. Corrected in both documents.

**Rejected.** Nothing new — `cmd/kbmerge` as a standalone binary (vs. an interactive `/kb merge` slash command) follows directly from the umbrella design's own stated precedent (`cmd/assay`'s relationship to the root package) and from this being an offline, cross-machine, file-level operation rather than something that belongs in a live chat session's `/kb` surface (`harvey-kb.7.md`).

**Consequences.** Implemented same-day, M1–M7 per `merge-tool-plan.md` (M8 full verification still open): `CollisionReport`, `MergeKnowledgeBases`, and `cmd/kbmerge` all in place with passing tests. **A real data-loss bug was caught during manual smoke-testing of `cmd/kbmerge`** (not by the unit tests, which didn't cover this until added after the fact): the originally-planned "-force resolves a collision by keeping the first-seen row" silently dropped every child row (observations, links) attached to the *losing* side's row too, not just the duplicate metadata row — because those children's `uuid` join has no merged parent left to attach to. Fixed with a new `ReconcileCollisions(bPath, collisions)` (M1.5, `knowledge_merge.go`), called by `cmd/kbmerge` before `MergeKnowledgeBases` whenever `-force` is set: rewrites `b`'s colliding rows to share `a`'s `uuid`, so both sides' children correctly merge into the one surviving parent. Reconfirmed via a real end-to-end smoke test through the actual `bin/kbmerge` binary (not just unit tests): before the fix, 2 source observations on a collided project merged down to 1; after, both survive. `merge-tool-design.md` and `merge-tool-plan.md` both corrected in place to document `ReconcileCollisions` and the collision-handling rationale. M8 complete same day: `go build`/`go vet`/`go test ./...` all clean; `go test -race` blocked by the same pre-existing Raspberry Pi ThreadSanitizer/VMA-width issue noted in the UUID migration entry (not a regression). Manual two-copy merge against the real, already-migrated `agents/knowledge.db` (two copies, one distinguishing observation added to each) merged cleanly with correct counts throughout — and surfaced an unrelated, pre-existing data-quality finding: 24 of 101 `observation_concepts` rows in the live database reference `observation_id`s that no longer exist (dangling links, most likely from some historical delete that ran without `PRAGMA foreign_keys=ON` active on that connection — SQLite's FK enforcement is per-connection, not persisted in the file). `MergeKnowledgeBases`'s join-based copy naturally excludes unresolvable links rather than propagating them, so this didn't affect the merge's correctness, but the 24 orphaned rows still exist in the live, unmerged `agents/knowledge.db` today — logged separately as a `finding` observation in `agents/knowledge.db` itself; not fixed here, out of scope for this session. Still blocked on actually running the merge against `wren.local`'s real data until that machine also has the UUID migration applied (that machine hasn't run it yet, per the previous entry's consequences).
...
**Consequences.** Implemented same-day, W1–W6 per `uuid-migration-plan.md`: nine new `kbAlterStmts`/`kbSourcesAlterStmts` entries, a `backfillUUIDs` helper (backfill + unique index per table, in `OpenKnowledgeBase`), and `uuid.NewV7()` + `os.Hostname()` stamped into all four `Add*` insert paths. `github.com/google/uuid` promoted from `// indirect` to a direct `go.mod` dependency. Nine new tests in `knowledge_test.go` (four backfill/idempotency, four insert-path, one conflict-preserves-uuid) confirmed red before implementation, green after. Verified against the real `agents/knowledge.db` on this machine: all 105 existing rows (3 projects, 79 observations, 17 concepts, 6 sources) backfilled to a non-empty `uuid`, and all 3 pre-existing projects correctly show `origin_host="unknown"`. `go test ./...` and `go vet ./...` clean. `go test -race` could not be verified on this machine — ThreadSanitizer fails immediately with `unsupported VMA range: Found 47 - Supported 48`, a kernel/hardware limitation on this Raspberry Pi confirmed pre-existing (identical failure on unmodified `main` via `git stash`), unrelated to this change. No merge tool, module extraction, or JSON-L export yet — all remain out of scope and deferred per the sequencing already decided in `../knowledge_db_merge_design.md`.
...
**Decision.** Added `probeClientReachable(client LLMClient) (reachable, checked bool)` (`chunk_analyzer.go`) — type-asserts to `*AnyLLMClient` and dispatches on `ProviderName()` to the same local health-probe functions already used elsewhere in the codebase (`ProbeOllama` for "ollama"; `ProbeLlamafile`/`probeLlamaCpp` — both via `LlamafileHealthURL(ac.BackendURL())`, since `BackendURL()` for these two already carries a `/v1` suffix the bare-base-URL probes don't expect — for "llamafile"/"llamacpp"). `checked=false` for cloud providers (no local health endpoint to probe) and any non-`*AnyLLMClient` (test doubles) — callers must treat `checked=false` as "nothing to verify," never as a failure. Called once, at the very top of `RunChunkedAnalysis`, before the map-phase loop begins — not duplicated at each of the three call sites (`cmdReadChunks`, `injectOrChunk`, `read_file`'s chunking guard). All three already have their own reasonable error-handling path for a `RunChunkedAnalysis` error, so an early, single error return flows through each unmodified, giving the "fail immediately with one clear message" behavior the TODO asked for without touching any of the three call sites at all.

**Rejected.** Duplicating the probe at each call site instead of once inside `RunChunkedAnalysis` — would triple the code for no benefit, and risks the three copies drifting (the same class of problem this whole thread has repeatedly found and fixed elsewhere, e.g. the tool-call-dispatch and context-percentage duplication fixed in the harness-prerequisite-refactor cycle).

**Consequences.** TDD-first: `TestRunChunkedAnalysis_FailsFastWhenBackendUnreachable` (`chunk_analyzer_test.go`), using a real `*AnyLLMClient` pointed at a guaranteed-unreachable address (`http://127.0.0.1:1`) — confirmed red first (all chunks + synthesis attempted, ~4s of real HTTP timeouts before erroring, matching the exact reported symptom), then green (fails immediately, no chunk-processing progress lines at all). Every other existing `RunChunkedAnalysis`/`read_file`/`injectOrChunk`/`cmdReadChunks` test — all built on `mockLLMClient`/`seqMockClient` test doubles, not `*AnyLLMClient` — passes unmodified, since `probeClientReachable` correctly returns `checked=false` for them and the preflight is a no-op. Full suite green except the same pre-existing, unrelated `TestCmdModelList_ShowsLlamafileEntries` failure noted in every entry above. `go vet ./...` and `gofmt -l` both clean.
...
Checked that premise before applying it. `chatInternal`'s logging goes through the client's own `DebugLog` field (`a.DebugLog.LogLLMRequest(...)`, `anyllm_client.go:162`) — nil-safe, but only *correct* when that field is actually wired to the real, open `*DebugLog`. Traced every path that can produce the `client` passed into `RunChunkedAnalysis` from `cmdReadChunks`/`injectOrChunk` (both use `resolveDispatchTarget`, Direction D): `a.Client` (the live agent client) is wired once at startup and is fine, but `resolveDispatchTarget`'s **route-registry branch** (`clientForEndpoint`) and **local-switch branch** (`attemptModelSwitch`'s Ollama case, `newOllamaLLMClient` with no `DebugLog` param) both construct fresh `*AnyLLMClient`s that are never wired to anything. Naively deleting the caller-side logging per the TODO's literal suggestion would have silently taken these paths from "logged once (by the caller)" to "logged never" — trading a real, reported bug for a quieter, worse one, specifically for `@mention`-dispatched chunk analysis.

**Decision.** Two-part fix. (1) `dispatch_target.go` gained `wireDebugLog(client LLMClient, dbg *DebugLog) LLMClient` — sets `.DebugLog` when `client` is an `*AnyLLMClient`, no-ops otherwise (e.g. test doubles) — applied at all three of `resolveDispatchTarget`'s return points (route branch, already-active branch, local-switch branch), so every client it can ever hand back is guaranteed to log its own calls correctly before any caller-side logging is removed. (2) Only then: removed the now-redundant `dbg *DebugLog` parameter entirely from `RunChunkedAnalysis` (not just its call sites — the parameter itself, since after removing every use of it the parameter would have been dead), and all six internal `dbg.LogLLMRequest`/`LogError`/`LogLLMResponse` call sites (map-phase request/error/response, reduce-phase request/error/response). Updated all three production callers (`commands.go`, `file_inject.go`, `builtin_tools.go`) and all eleven test call sites (`chunk_analyzer_test.go`) to match the new five-argument signature.

**Rejected.** Applying the TODO's literal suggestion as-is (delete caller-side logging, leave `resolveDispatchTarget` unwired) — would have introduced a silent logging regression for the `@mention`-dispatch path, discovered only by tracing where `client` actually comes from rather than assuming the two call sites always see the same, already-wired client.

**Consequences.** TDD-first: `TestResolveDispatchTarget_RouteEndpoint_WiresDebugLog`, `TestResolveDispatchTarget_LocalSwitch_WiresDebugLog` (both confirmed red — `ac.DebugLog` was nil after resolution — then green). `chunk_analyzer_test.go`'s existing eleven tests (map/reduce/error/recorder/progress/message-isolation/system-prompt coverage) all pass unmodified in behavior, only their call signatures updated — confirming the removal is behavior-preserving for the actual chunk-analysis logic. Full suite green except the same pre-existing, unrelated `TestCmdModelList_ShowsLlamafileEntries` failure noted in every entry above. `go vet ./...` and `gofmt -l` both clean.

**Related finding, not fixed (out of scope for this session's ask):** `builtin_tools.go`'s `read_file` tool has its own pre-read chunking guard with the *exact same* cosmetic-only `@mention` bug (Bug 1) that Direction D fixed for `/read-chunks` and `injectOrChunk` — it parses `@mention` to relabel `ChunkAnalysisParams.Model` for recording, but always dispatches via `a.Client`, never the mentioned model. Missed during Direction D's original audit (only two of the three chunk-analysis-triggering call sites were found and fixed at the time). Noted in `TODO.md` as a follow-up; not fixed here since it wasn't part of what was asked this session.

---

## 2026-07-13 — `pickBackend` startup picker gains `.gguf`/llama.cpp models (TODO.md bug fix)

**Context.** `TODO.md`: "I have both Llamafile and gguf models in ~/Models on my Mac, but the gguf models are not listed as an option (llama.cpp is installed)." Traced the exact code path before proposing a fix. `selectBackend` (`backend_startup.go`) has three cases: Case 0 adopts an already-running `llama-server` via a live health probe (`Detect()`); Case 2 (`pickBackend`) shows a combined picker whenever any llamafile is registered. Initial hypothesis was that Case 0's gate condition (`len(a.Config.LlamaCpp.ModelsDir) > 0 || a.Config.LlamaCpp.URL != ""`) was too narrow — checked more carefully and found this was wrong: `DefaultConfig()` always sets `LlamaCpp.URL = "http://127.0.0.1:8081"`, and the YAML-merge logic (`config.go:899-900`) only ever overwrites it with a non-empty value, never clears it — so the gate is satisfied in practice regardless of what the user's `harvey.yaml` says. Case 0 already runs its probe every startup; it just silently finds nothing when no server is running, which is the common case for a user who has `.gguf` files on disk but hasn't started `llama-server` themselves. Corrected the diagnosis before implementing rather than fixing a gate that wasn't actually broken.

The real, sole root cause: `pickBackend`'s option-building only ever considers registered llamafiles, disk-scanned unregistered llamafiles, and live Ollama models — there is no code path for `.gguf`/llama.cpp models at all. `/model list`/`/model use` (`aggregateModels`, `model_picker.go`) already correctly combine all three backends (confirmed via the existing, passing `TestCmdModelList_ShowsGGUFEntries`) — the startup flow simply predates that later unification and was never brought in line with it.

**Decision.** Added a `.gguf` disk-scan branch to `pickBackend`, mirroring the existing "llamafiles found on disk but not yet registered" block exactly: scan via `NewLlamaCppBackend(a.Config, agentsDir).ListModels()`, dedupe against `seenPaths`, append as `option{kind: "llamacpp", ...}`. The `option` struct's `kind` field now documents three values instead of two. When the user picks a `"llamacpp"` option, the chosen-option switch calls the already-existing `startLlamaCppModelPath(a, chosen.path, out)` (`backend_llamacpp.go:462`) — no new start handler needed, since that function already does everything required (stop current backend, start `llama-server` for the given path, wire `a.Client`/`a.Backend`, clear stale `Llamafile.Active`, probe+cache).

**Rejected.** "Fixing" Case 0's gate condition — considered per the initial hypothesis, but tracing `DefaultConfig()`/the YAML merge logic showed there was no real bug there; would have been a no-op change justified by a mistaken premise.

**Consequences.** TDD-first: `TestPickBackend_ListsGGUFModels` (`llamafile_test.go`), mirroring `TestPickBackend_ListsUnregisteredDiskModels`'s existing pattern (temp `ModelsDir`, unreachable URLs elsewhere to isolate the picker's output). Confirmed red first — the test's initial failure was itself revealing: with no llamafile/Ollama/gguf options at all, `pickBackend` fell into its "no models found" first-run wizard branch rather than the numbered picker, confirming the gap was structural, not cosmetic. Confirmed green after the fix. Full suite green except one already-diagnosed, unrelated pre-existing failure (`TestCmdModelList_ShowsLlamafileEntries`) — noted below as a related but out-of-scope finding, not fixed as part of this change since it wasn't asked for. `go vet ./...` and `gofmt -l` both clean.

**Related finding, not fixed (out of scope for this session's ask):** `TestCmdModelList_ShowsLlamafileEntries` — the one failure that has been called "pre-existing, unrelated" throughout every prior entry in this log — turns out to have a diagnosable root cause, found while working in this same area: it doesn't set `a.Config.Llamafile.ModelsDir` to an isolated temp directory the way `TestPickBackend_ListsUnregisteredDiskModels`/`TestPickBackend_ListsGGUFModels` do, so `aggregateModels`'s llamafile disk-scan leaks whatever real `.llamafile` files happen to sit in `$HOME/Models` on the machine running the test — which on this sandbox includes real entries from prior Henry-related benchmarking work, none of which is literally `"bonsai-8b"` (the fixture's exact name), hence the mismatch. A one-line fix (isolate `ModelsDir` to `t.TempDir()`, matching the sibling tests) would resolve it; not applied here since it wasn't part of what was asked.

---

## 2026-07-13 — Direction F (harness-template skills): blocked on a prerequisite that doesn't exist — not pursued

**Context.** `harness-engineering-exploration.md` Direction F: bundle guides *and* sensor scaffolding together into an invokable skill, so loading a skill can provision sensor configuration into the workspace, not just inject markdown into the system prompt. The exploration doc itself flagged this as sequenced last, downstream of Direction A having more sensors to template — but auditing before designing found the gap is more fundamental than "not enough sensors yet."

Checked `commands_skill.go`'s `skillLoad` (the `/skill load` implementation): it does exactly one thing — `a.AddMessage("user", "[skill: name]\n\n"+skill.Body)`. Pure markdown-into-context injection; no hook exists anywhere for a skill to provision anything else into the workspace. More fundamentally, checked whether *any* sensor in Harvey today is config-driven at all: `gofmt` `Check()` and `go vet` (Direction A's only two shipped sensors, `computational-sensors-design.md`) are both hardcoded — neither reads an external ruleset file. The only sensor-related field anywhere in `Config` is `SensorInjectFormatFindings`, a single visibility-gating bool, not a ruleset. **There is no such thing as "sensor configuration" in Harvey yet, for any sensor** — so "scaffold sensor config alongside guide markdown" has no artifact to scaffold, independent of how many sensors exist.

**Decision.** Confirmed with the user (2026-07-13): stop here, document the finding, build nothing. Direction F is blocked on a genuine prerequisite — a configurable sensor (e.g. a `gocyclo`-style complexity check with a per-project threshold, named as a Direction A candidate but never built) — not merely "more sensors to choose from." Building F's mechanism now, against a hypothetical config shape with nothing real to provision, was considered and rejected: it would be speculative infrastructure with no working example to validate its design against, the same anti-pattern already avoided in Direction B (item 1, content-aware guide selection — no path-scoped skill existed to gate) and Direction D (chunk analysis correctly left tool-free rather than force-fit into the consolidation).

**Rejected.** Expanding scope to build a configurable sensor first, then F on top of it — a real, legitimate path, but two increments' worth of work the user did not ask for this session; left as the natural next step whenever Direction A itself grows a config-driven sensor for its own reasons. Designing F's provisioning mechanism generically without a real sensor to prove it against — rejected for the same reason Direction E's "mechanism only" choice was *not* applied here: Direction E had `--rag-compare`'s existing shape to mirror as a concrete template; F has no analogous existing mechanism in Harvey to mirror, so building it now would be closer to invention than extraction.

**Consequences.** No code changes. `harness-engineering-exploration.md`'s Directions A–F are now all either implemented (A, B, D, E) or explicitly resolved as blocked-not-pursued (F) — this closes out the thread this document opened. Revisit F only if/when a config-driven sensor exists in Harvey for independent reasons; at that point F would have a first real artifact to scaffold and the "mechanism only" pattern from Direction E likely applies then.
